...
std::unordered_map<int, int> STrack::static_track_id_dict;

STrack::STrack(const std::vector<float>& tlwh_, const float& score, const int& cls_id)
{
	_tlwh.resize(4);
...
void STrack::init_trackid_dict(const int n_classes)
{
	for (int i = 0; i < n_classes; ++i)
	{
		STrack::static_track_id_dict[i] = 0;
	}
}
...
void STrack::activate(byte_kalman::ByteKalmanFilter& kalman_filter, int frame_id)
{
	...
	this->track_id = STrack::next_id(this->class_id);
...
int STrack::next_id(const int& cls_id)
{
	STrack::static_track_id_dict[cls_id] += 1;
	return STrack::static_track_id_dict[cls_id];
}
